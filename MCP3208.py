...
class MCP3208:
	...
	"""
	def __init__(self,bus:int=0,device:int=0,mode:bytes=0b00,bits_per_word:int=8,max_speed_hz:int=500000):
	...
	def read_ADC(self,channel_mumber:int=None)->int:
		...
		"""
            The channel numbering starts from CH0 through to CH7, while the human readable format starts from 1 to 8.
            The human readable format is what this method accepts. Therefore, to compensate for the offset,
            we will subtract 1 from the human readable parameter that is recieved.
        """
		channel_mumber -=1
		...
		# prepare the commands to be sent to MCP3208
		...
		data = self.spi.xfer2([cmd1, cmd2, cmd3])
		data[1] = data[1] & 0x0F;
